...
def save_confusion_matrix(y_true, y_pred, model_name, output_path):
    ...
    # Using ConfusionMatrixDisplay for easier plotting
...
class BertBaselineCNN(nn.Module):
...
def evaluate(model, loader, device):
    ...
    # AUC (Handle cases with missing classes gracefully)
    try:
...
if __name__ == "__main__":
    ...
    print("\n--- Training Baseline Model (Fixed BERT-CNN) ---")
    ...
    baseline_model = BertBaselineCNN(num_classes=3, freeze_bert=True).to(device)
    ...
    torch.save(baseline_model.state_dict(), "modelling/models/baseline_bert_cnn.pt")
    
    # Final Eval Baseline
    base_acc, base_f1, base_auc, base_y, base_probs = evaluate(baseline_model, base_loader_test, device)
    save_roc_curve(base_y, base_probs, "Baseline BERT-CNN", "modelling/models/baseline_bert_cnn_roc.png")

    base_preds = np.argmax(base_probs, axis=1)
    save_confusion_matrix(base_y, base_preds, "Baseline BERT-CNN", "modelling/models/baseline_bert_cnn_cm.png")
    ...
    torch.save(final_model.state_dict(), "modelling/models/best_bert_cnn.pt")
    
    # Final Eval Optimized
    final_acc, final_f1, final_auc, final_y, final_probs = evaluate(final_model, final_test_loader, device)
    save_roc_curve(final_y, final_probs, "Optimized BERT-CNN", "modelling/models/best_bert_cnn_roc.png")
    
    final_preds = np.argmax(final_probs, axis=1)
    save_confusion_matrix(final_y, final_preds, "Optimized BERT-CNN", "modelling/models/best_bert_cnn_cm.png")
    ...
    if final_f1 > base_f1:
        print("Hyperparameter tuning improved model performance (AUC).")
    else:
